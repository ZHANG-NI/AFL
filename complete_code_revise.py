...
import os
import re
from code_judgement import code_overall_jud, code_jud
from code_generation import revise_prompt, read_vrp_file
client = OpenAI(api_key="")

def code_analyse(vrp_path, vrp_text, problem_desc, constraints, specific_name, input_def, output_def, objective, code, error_msg, again=0, model="gpt-4.1"):
    context = (
    ...
    task_map = {
        ...
        "initial": (
            ...
            "       - If fuel is not enough to go to other customer or depot, go to the station respect to all constrains."
            ...
            "       - If no feasible customer can be selected as the next node from customer or station due to capacity, time, or distance limit, the vehicle go back to the depot if routes are closed( and can through at most onecharging station if fuel is not enough to go back to depot respect to all constrains), and assign vehicle for new route.\n"
            ...
            "       - Before come back to depot, check again the fuel is enough or not, if not, go to one station respect to all constrains\n"
            "       - For node that can not find the initial solution, must try all conbination of station in deopt->neatest station of node->node->neatest station of node(->depot if closed route) and deopt->node->station(->depot if closed route) which comfirm two station simultaneously instead one by one, and choose the feasible one respect to all constrains.\n"
        ...
        "insert": (
            ...
            f"    - The chosen insertion minimizes the cost increase while maintaining feasibility under {constraints}.\n"
            ...
            "    - When try to open new route, must try all combination of station in deopt->station->node->station(->depot if closed route) and deopt->node->station(->depot if closed route) which comfirm two station simultaneously instead one by one, and choose the feasible one.\n"
    ...
    selected_rules = "".join(task_map.get(name, "") for name in func_names[:])
    base_info = (
        ...
        "The code are generate using following rules:\n"
        f"In 'initial' function, each step must respect to {constraints}.\n"
        f"In 'insert' function, the chosen insertion must **minimizes the cost increase** while maintaining feasibility under {constraints}.\n"
        + selected_rules
        + "Use the provided .vrp file content as the **only ground truth** for analyse. "
    ...
    return (
        ...
        + context
        + base_info
        + "Assume that in this VRP file format, whenever a section is present (e.g., NODE_COORD_SECTION, DEMAND_SECTION, TIME_WINDOW_SECTION, SERVICE_TIME_SECTION), "
        "it must contain entries for **all nodes, including the depot**, and no extra entries.\n"
        f"Assume that the VRP file provides exactly all elements listed in {input_def}.\n"
        f"Assume that this code is only intended to work for {specific_name}, {problem_desc}.\n\n"
        ...
        #" If initial solution invalid in error message, and also it seems cannot find any infeasible route with greedy stragety, change the greedy to insert heuristic strgety in initial function, which create route depot(-> depot if need to go back to depot) and insert node to depot."
        "The code execution failed and produced the following error message:\n"
        f"{error_msg}\n\n"
        "Your task:\n"
        "Analyse the error message, explain why the error happened.\n"
        f"- If 'Electricity' activate, If the error message explicitly states both 'Initial solution is not valid' and information about route exceeds the fuel capacity when returning from a customer to the depot, then the algorithm must try all combination of station in depot → station → customer → station (→ depot if closed route) and deopt->station->node(->depot if closed route), while strictly respecting all constraints(station can be same or different) and choosing the feasible one. If not, don't give this suggestion.\n"
        f"- If 'Electricity' activate, If the error message explicitly states time out, then the algorithm must try all combination of station in depot → station → customer → station (→ depot if closed route) and deopt->station->node(->depot if closed route) which comfirm two station simultaneously instead one by one, while strictly respecting all constraints(station can be same or different) and choosing the feasible one. If not, don't give this suggestion.\n"
        "- If 'Electricity' activate, If no feasible insertion exists in insert function, must create a new route for this node that satisfies all constraints. When try to create a new route, must try all combination of station in deopt->station->node->station(->depot if closed route) and deopt->node->station(->depot if closed route) which comfirm two station simultaneously instead one by one, and choose the feasible one(station can be same or different).\n"
        "- Provide clear and concrete suggestions for how to fix or improve the code.\n\n"
        "⚠️ Formatting rule: For easier parsing, your explanation and suggestions must be written strictly "
        "in plain text on a single line, without using any line breaks (`\\n`) or additional colons (`:`) except the required `jud1:` prefix.\n\n"
        "Output format must be exactly 1 line:\n"
        "1) jud1: explanation and suggestions\n"
    )



def revise_prompt1(vrp_path, vrp_text, problem_desc, constraints, specific_name, input_def, output_def, objective, prev_code, jud, error_msg)-> str:
    context = ""
    if prev_code:
        context = (
...
def extract_jud1(reply: str) -> str:

    lines = [line.strip() for line in reply.splitlines() if line.strip()]
    for line in lines:
        if line.lower().startswith("jud1"):
            return line
    raise ValueError("❌ Could not find 'jud1:' in GPT reply:\n" + reply)
...
def revise_code(vrp_path, problem_desc, constraints, specific_name, input_def, output_def, objective, code, error_msg, model="gpt-4.1"):
    vrp_text = read_vrp_file(vrp_path)
    analyse_promote = code_analyse(vrp_path, vrp_text, problem_desc, constraints, specific_name, input_def, output_def, objective, code, error_msg, model)
    analyse_reply = ask_gpt(analyse_promote, model)
    analyse_reply = extract_jud1(analyse_reply)
    print(f"error_analyse: {analyse_reply}")
    prompt = revise_prompt1(vrp_path, vrp_text, problem_desc, constraints, specific_name, input_def, output_def, objective, code, analyse_reply, error_msg)
    ...
    right, jud = code_overall_jud(vrp_path, vrp_text, problem_desc, constraints, specific_name, input_def, output_def, objective, code)
    prefix = "√" if right else "x"
    print(f"{prefix} run overall Validation result: {right}, {jud}")
    while right is False:
        cost_task+=1
        prompt = revise_prompt(vrp_path, vrp_text, problem_desc, constraints, specific_name, input_def, output_def, objective, code, jud)
        raw_code = ask_gpt(prompt)
        code = extract_code(raw_code)
        with open(out_file, "w", encoding="utf-8") as f:
            f.write(code)
